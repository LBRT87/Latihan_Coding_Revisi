...
import CreateIceCream from "./CreateIceCreamPage";
...
export default function DashboardA({ user, onLogout }: DashboardAProps) {
    const [activeTab, setActiveTab] = useState<'Landing' | 'Profile' | 'Dashboard' | 'Create Ice Cream' | 'Transaction'>('Profile');
    const navContent = {
        nav1: 'Dashboard',
        nav2: 'Create Ice Cream',
        nav3: 'Transaction'
    } as NavContent
    return (
        <div className="w-full bg-slate-800 p-6 rounded-2xl shadow-xl space-y-4 h-full">
            <Navigation user={user} onLogout={onLogout} activeTab={activeTab} onChangeTab={(tab) => setActiveTab(tab as 'Landing' | 'Profile' | 'Dashboard' | 'Create Ice Cream' | 'Transaction')} navigation={navContent}/>
            
            {activeTab === 'Landing' && (<Landing carousel={{img1: "hal",img2: "hal",img3: "hal",img4: "hal",img5: "hal"}}/>)}

            {activeTab === 'Profile' && ( 
                <div className="max-w-2xl mx-auto bg-slate-800 p-6 rounded-xl border border-slate-700 max-h-max">
            ...
            {activeTab === 'Dashboard' && (<Shop iceCream={[]}/> )}

            {activeTab === 'Create Ice Cream' && (<CreateIceCream/>)}

            {activeTab === 'Transaction' && (<History transaction={{actions: "a", customer: "wilson", date:"1", status:"te", total:123 }}/>)}
        </div>
    );
}